...
_Correct! The public directory is more of a convention than a rule. We can name our static file directory whatever we want as long as it's reflected in the static middleware._

You have to name that directory public , as it is specifically what a server looks for when serving front-end assets.
_No, we dictate what directory serves our static front-end assets using the express.static() middleware. We can use whatever name we'd like as long as it makes sense to us_

#### Question 2

The following code will work on our machines, but what's the potential problem with it?
...
We aren't indicating whether this is a GET or POST request to the server.

There are no potential problems here.

The URL is an absolute path to the local development server, meaning we'd have to change it for a production environment.
_Correct! While this would still work in development, it would add the complexity of differing server URLs between environments. It's better to use a relative path such as /api/animals because it will still work with a different base server URL._

#### Question 3

True or false? When serving a front end using Express, we can still use the VS Code "Open in Browser" tool to open an HTML file in a browser.

False
_Correct! If we open a file using "Open in Browser" instead of using its server path, any HTTP calls to the server won't work, because the base URL won't be the server's URL._
True